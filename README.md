...
デフォルトは %APPDATA%\CorvusSKK で、設定ダイアログの「個人辞書」タブのユーザー辞書バックアップディレクトリで設定可能です。
...
* 設定ダイアログの「個人辞書」タブの「辞書管理プロセス」で「終了」ボタンを押す。
...
* 設定ダイアログの「個人辞書」タブの「辞書管理プロセス」で「実行」ボタンを押す。
* IME OFF → ON
* 仮名漢字変換開始
* 補完開始
...
* 辞書管理プロセスが終了するとき
* スリープ、休止状態から復帰したとき
* 設定ダイアログの「個人辞書」タブの「ユーザー辞書バックアップ」で手動バックアップを実行したとき
...
設定ダイアログのタブ「[個人辞書](#ユーザー辞書)」「[トグル](#キー設定onoff)」「[仮想キー](#キー設定文字キー設定仮想キー)」のそれぞれ右上にある「仮想ｷｰﾃｽﾄ」エディットボックスにフォーカスがある状態でキー入力すると仮想キーコードを表示します。
